...
export default function AdminBulkCreatePage() {
  ...
  if (stage === 'result') {
    return <BulkResult created={created} batchNo={batchNo} onUploadAnother={reset} viewAllHref="/admin/shipments" viewAllLabel="View all shipments" />;
  }
      ...
      <div className="max-w-[1400px] mx-auto">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="text-gray-500 mt-1">
              Fix any highlighted cells, choose the client, then create. {validation.rowCount} row{validation.rowCount === 1 ? '' : 's'} found.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="mt-6">
          <BulkReviewGrid
            response={validation}
            clients={clients}
            createUrl="/api/admin/shipments/bulk"
            onCreated={(c, b) => {
              setCreated(c);
              setBatchNo(b);
              setStage('result');
    ...
    <div className="max-w-3xl mx-auto">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">Upload an Excel file to create multiple shipments for a client at once.</p>
        </div>
        <Link
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 sm:p-6 mt-6">
        ...
        <ol className="mt-3 space-y-1.5 text-sm text-gray-600 list-decimal list-inside">
          ...
          <li>Fill in one row per shipment (pickup, delivery, parcels, mode, etc.).</li>
          <li>Upload it below and review the rows.</li>
          <li>Choose the client to book on behalf of, then create them all.</li>
        </ol>
        <a
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 sm:p-6 mt-4">
        ...
        <div className="flex items-center justify-end gap-3 mt-5">
          <Link href="/admin/shipments" className="px-5 py-2.5 text-sm font-semibold text-brand-gray hover:text-brand-dark transition-colors">
            Cancel
          </Link>
          <button
            type="button"
